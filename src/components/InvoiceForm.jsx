...
export default function InvoiceForm() {
  ...
  }, [isEditMode, id, fetchInvoiceById, fetchClientsByAdmin]);
  ...
  useEffect(() => {
    ...
    if (currentInvoice && isEditMode) {
      ...
      setFormData({
        numFacture: currentInvoice.numFacture || '',
        client: currentInvoice.client || '',
        dateExpiration: formatDate(currentInvoice.dateExpiration) || formatDate(new Date(new Date().setMonth(new Date().getMonth() + 1))),
        paid: parseFloat(currentInvoice.paid) || 0,
        status: currentInvoice.status || 'en attente',
        type: currentInvoice.type || 'Personne',
        total: currentInvoice.total || 0,
        articles: currentInvoice.articles || {}
      });

      // Vérifier si les articles existent et les charger dans le cartState
      if (currentInvoice.articles && Array.isArray(currentInvoice.articles)) {
        setCartState({
          articles: currentInvoice.articles,
          taxe: tauxTaxe * currentInvoice.total / (1 + tauxTaxe),
          sousTotal: currentInvoice.total / (1 + tauxTaxe),
          total: currentInvoice.total
        });
      } else if (currentInvoice.articles && currentInvoice.articles.articles) {
        setCartState({
          articles: currentInvoice.articles.articles || [],
          taxe: currentInvoice.articles.taxe || 0,
          sousTotal: currentInvoice.articles.sousTotal || 0,
          total: currentInvoice.articles.total || 0
        });
      }
    }
  }, [currentInvoice, isEditMode, tauxTaxe]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Créer une copie mise à jour du formData avec les articles actuels du cartState
    ...
    if (isEditMode) {
      result = await updateInvoice(id, updatedFormData);
    } else {
      result = await createInvoice(updatedFormData);
    }
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-8 flex justify-end">
            ...
            >
              {invoiceLoading ? 'Chargement...' : isEditMode ? 'Mettre à jour' : 'Créer'}
            </button>
          </div>
        </form>
      </div>
