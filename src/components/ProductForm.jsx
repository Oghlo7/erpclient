...
export default function ProductForm() {
  ...
  const { numCat } = useParams();
  const isEditMode = !!numCat;
  ...
  useEffect(() => {
    ...
    // Si en mode édition, récupérer les données du produit
    if (isEditMode) {
      fetchProductById(numCat);
    }
  }, [isEditMode, numCat, fetchProductById, fetchCategoriesByAdmin]);
  
  useEffect(() => {
    // Remplir le formulaire avec les données du produit lorsqu'elles sont disponibles
    if (currentProduct && isEditMode) {
      setFormData({
        nom: currentProduct.nom || '',
        category: currentProduct.category || '',
        devis: currentProduct.devis || 'USD',
  ...
  const validateForm = () => {
    ...
    if (!formData.price || formData.price <= 0) {
      errors.price = 'Le price doit être supérieur à 0';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode) {
      result = await updateProduct(numCat, formData);
    } else {
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                <span className="text-red-500">*</span> Catégorie de produit
              </label>
              <select
                name="category"
              ...
              >
                ...
                {activecategories.map(category => (
                  <option key={category.numCat} value={category.numCat}>
                    {category.nom}
                  </option>
                ))}
              </select>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                <span className="text-red-500">*</span> devis
              </label>
              <select
                name="devis"
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full border ${formErrors.price ? 'border-red-500' : 'border-gray-300'} rounded-md px-3 py-2`}
                  step="0.01"
                  min="0"
                />
              </div>
          ...
          <div className="mt-6 flex justify-end">
            ...
            >
              {productLoading ? 'Chargement...' : isEditMode ? 'Mettre à jour' : 'Créer'}
            </button>
          </div>
        </form>
      </div>
